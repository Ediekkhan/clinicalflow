...
def test_patient_login_returns_profile_and_session_like_response() -> None:
    with TestClient(app) as client:
        ...
        assert "__Host-cf_session" in response.cookies
        assert "__Host-cf_refresh_token" in response.cookies
...
def test_refresh_rotates_session_and_logout_revokes_it() -> None:
    with TestClient(app) as client:
        ...
        old_refresh = login.cookies["__Host-cf_refresh_token"]
        refreshed = client.post("/api/v1/auth/refresh")
        new_refresh = refreshed.cookies["__Host-cf_refresh_token"]
