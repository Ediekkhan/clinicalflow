              ...
              <div className="mt-6 grid gap-4">
                {fields.map(([name, label, type]) => (
                  <label key={name} className="grid gap-1.5">
                    ...
                    <input required={name === 'organization_name' || name === 'work_email'} type={type} value={form[name]} onChange={(event) => setForm((current) => ({ ...current, [name]: event.target.value }))} className="rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-[#2563EB] focus:ring-2 focus:ring-blue-100" placeholder={label} />
                  </label>
                ))}
                {status === 'error' ? <p className="text-sm font-semibold text-rose-600">Could not save your request. Please try again.</p> : null}
                <button disabled={status === 'saving'} className="rounded-xl bg-[#2563EB] px-5 py-3 text-sm font-semibold text-white disabled:bg-slate-300">{status === 'saving' ? 'Sending…' : 'Request Demo'}</button>
              </div>
            )}
          </form>
        </section>
