  ...
  async function load() {
    ...
    try {
      const [response, profile] = await Promise.all([
        api.get('/api/v1/platform/facility-verifications') as Promise<{ items?: Verification[] }>,
        api.get('/api/v1/auth/admin/me') as Promise<{ role?: string }>,
      ]);
      setItems(Array.isArray(response.items) ? response.items : []);
      setCanReview(profile.role === 'facility_verifier' || profile.role === 'verification_supervisor');
    }
