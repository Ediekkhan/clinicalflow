    ...
    <HospitalShell>
      <main className="mx-auto grid max-w-7xl gap-5 p-4 md:p-6">
        <div>
          ...
          <h1 className="font-display text-4xl text-[#111827] md:text-5xl">{settings.facility_name ? `${settings.facility_name} account controls` : 'Account controls'}</h1>
        </div>
        ...
        <section className="rounded-card border border-[#E5E7EB] bg-white p-4 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <button onClick={() => void patchSettings({ intake_paused: !paused })} className={cn('front-desk-target inline-flex items-center gap-2', paused ? 'bg-[#DC2626] text-white' : 'bg-[#2563EB] text-white')}>
          ...
          <div className="rounded-card border border-[#E5E7EB] bg-white p-4 shadow-sm">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {smsRoutes.length ? smsRoutes.map((route) => (
                <button key={route} className="front-desk-target border border-[#2563EB] bg-[#e0f2fe] text-[#2563EB]">{route}</button>
              )) : <p className="text-sm text-[#6B7280]">No SMS route configured</p>}
            </div>
          </div>
          <div className="rounded-card border border-[#E5E7EB] bg-white p-4 shadow-sm">
            <div className="flex items-center justify-between gap-3">
              ...
              <button onClick={() => void patchSettings({ whatsapp_enabled: !whatsappEnabled })} className="rounded-lg p-3 text-[#2563EB] hover:bg-[#e0f2fe]" aria-label="Toggle WhatsApp channel">
          ...
          {settings.staff?.length ? (
            <div className="mt-4 overflow-hidden rounded-card border border-[#E5E7EB]">
              {settings.staff.map((staff) => (
                <div key={staff.id ?? staff.name} className="data-row grid gap-2 border-b border-[#E5E7EB] last:border-b-0 sm:grid-cols-3">
                  <span className="font-medium text-[#111827]">{staff.name}</span>
                  <span className="inline-flex items-center gap-2 text-[#6B7280]">
                    <Shield className="h-4 w-4" />
                    {staff.role}
                  </span>
                  <span className="text-[#6B7280]">{staff.scope}</span>
                </div>
              ))}
            </div>
          ) : (
