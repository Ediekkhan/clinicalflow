    ...
    <main className="bg-slate-50">
      ...
      <section className="bg-[#0D1117] px-6 pb-20 pt-32 text-white">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-8 flex flex-wrap gap-3 text-sm text-blue-100">
            {['Real-Time Kanban', 'Multi-Specialist Routing', 'NDPA Compliant', 'WhatsApp Integration'].map((pill) => (
              <span key={pill} className="rounded-full border border-blue-800 bg-blue-950 px-4 py-2">{pill}</span>
            ))}
          </div>
          <Link href="/book-demo" className="mt-8 inline-flex rounded-xl bg-[#2563EB] px-6 py-3.5 font-semibold text-white">Book a Free Demo</Link>
        </div>
      </section>
      <section className="px-6 py-20">
        <div className="mx-auto grid max-w-6xl gap-4 md:grid-cols-2 xl:grid-cols-3">
          {([
            ...
            ['Analytics', 'Track wait times, top conditions, and utilization.', BarChart2],
      ...
      <section className="bg-white px-6 py-20">
        <div className="mx-auto grid max-w-5xl gap-4 md:grid-cols-3">
          {[
            ['Before', '45 mins', 'Average manual intake'],
            ['After', '8 mins', 'With SynaptiVerse triage'],
            ['Reduction', '82%', 'Faster first routing'],
          ].map((stat) => (
            <article key={stat[0]} className="rounded-2xl border border-slate-100 p-6 text-center">
              <p className="text-sm font-semibold text-slate-400">{stat[0]}</p>
              <p className="font-display mt-2 text-5xl text-slate-900">{stat[1]}</p>
              <p className="mt-2 text-sm text-slate-500">{stat[2]}</p>
            </article>
          ))}
        </div>
      </section>
