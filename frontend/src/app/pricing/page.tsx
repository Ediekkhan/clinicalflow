...
export default function PricingPage() {
  return (
    <main className="bg-slate-50 text-[#10231e]">
      <SiteLayout>
      ...
      <section className="px-6 pb-24" aria-labelledby="pricing-tiers-heading">
        <div className="mx-auto max-w-6xl">
          <h2 id="pricing-tiers-heading" className="sr-only">ClinicalFlow pricing tiers</h2>
          <div className="grid gap-6 lg:grid-cols-3 lg:items-stretch">
            {pricingTiers.map((tier) => (
              <article
                key={tier.name}
                aria-labelledby={`${tier.name.toLowerCase()}-tier-title`}
                className={`relative flex rounded-lg border bg-white p-6 shadow-sm transition duration-200 ${tier.popular ? 'border-[#0b5d4b] shadow-blue-100 hover:scale-105 hover:shadow-xl' : 'border-slate-200 hover:-translate-y-1 hover:shadow-md'}`}
              >
                {tier.popular ? (
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2 rounded-full bg-[#0b5d4b] px-4 py-1 text-xs font-bold uppercase tracking-wide text-white">
                    Most Popular
                  </div>
                ) : null}
                <div className="flex w-full flex-col">
                  <header>
                    <h3 id={`${tier.name.toLowerCase()}-tier-title`} className="text-xl font-black text-[#10231e]">{tier.name}</h3>
                    <p className="mt-4 flex items-end gap-2" aria-label={`${tier.name} plan price is ${tier.price} ${tier.cadence}`}>
                      <span className="font-display text-5xl font-black text-[#10231e]">{tier.price}</span>
                  ...
                  <ul className="mt-6 space-y-3" aria-label={`${tier.name} plan features`}>
                    {tier.features.map((feature) => (
                      <li key={feature} className="flex gap-3 text-sm leading-6 text-slate-700">
                        <CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-[#0b5d4b]" aria-hidden="true" />
                        <span>{feature}</span>
                      </li>
                    ))}
                  </ul>
                  ...
                  <footer className="mt-auto pt-8">
                    <Link
                      ...
                      className={`inline-flex min-h-12 w-full items-center justify-center rounded-lg px-4 py-3 text-sm font-bold transition-colors duration-200 ${tier.popular ? 'bg-[#0b5d4b] text-white hover:bg-[#073d33]' : 'bg-[#10231e] text-white hover:bg-slate-700'}`}
                    >
                      {tier.cta}
                    </Link>
                  </footer>
