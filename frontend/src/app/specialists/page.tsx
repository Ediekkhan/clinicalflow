      ...
      <section className="bg-[#073d33] px-6 pb-20 pt-32 text-white">
        <div className="mx-auto grid max-w-6xl gap-10 md:grid-cols-[1fr_0.6fr] md:items-end">
          <div>
            ...
            <p className="mt-5 max-w-2xl text-lg leading-8 text-slate-400">Join ClinicalFlow's growing network of specialists. Our AI triage engine sends you correctly routed patients so you spend less time on admin and more time on care.</p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href="/signup?type=specialist" className="rounded-xl bg-[#0b5d4b] px-6 py-3.5 font-semibold text-white">Register as a Specialist</Link>
      ...
      <section className="bg-white px-6 py-20">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-3">
            {([
              ['Register Independently', 'Set up your own practice on ClinicalFlow with your schedule and fees.', User],
              ...
              <article key={String(title)} className="rounded-2xl border border-slate-100 bg-slate-50 p-6">
                <Icon className="h-6 w-6 text-[#0b5d4b]" />
          ...
          <div className="mt-10 rounded-2xl border border-slate-100 bg-slate-50 p-6">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              {['Valid MDCN license', 'Practicing certificate', 'Government-issued ID', 'Bank account for disbursement', 'Device with camera for teleconsultation'].map((item) => (
                <p key={item} className="inline-flex items-center gap-2 text-sm text-slate-600"><CheckCircle2 className="h-4 w-4 text-[#0b5d4b]" />{item}</p>
              ))}
            </div>
          </div>
        </div>
