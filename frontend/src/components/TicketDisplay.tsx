  ...
  return (
    <section
      className={cn(
        ...
        seeing && 'animate-overtake-pulse border-emerald-300 bg-[#E6F4F0]',
      )}
    >
      <div>
      ...
      <div className="grid gap-2">
        ...
        <div className="mx-auto h-2 w-full max-w-sm overflow-hidden rounded-full bg-slate-100">
          <div className="h-full w-3/5 rounded-full bg-[#0D7A5F]" />
        </div>
      ...
      <div className="grid gap-3 rounded-card bg-[#F7F8FA] p-4 text-left text-sm text-[#111827]">
        <p className="inline-flex items-center gap-2">
          <Stethoscope className="h-4 w-4 text-[#0D7A5F]" />
          Specialist: Dr. Udo Okon
        </p>
        <p className="inline-flex items-center gap-2">
          <DoorOpen className="h-4 w-4 text-[#0D7A5F]" />
          Room 4 · {ticket.assigned_specialty}
        </p>
        <p className="inline-flex items-center gap-2">
          <Clock className="h-4 w-4 text-[#0D7A5F]" />
          Est. Wait: ~{ticket.wait_minutes} mins
        </p>
      </div>
      <div className="flex flex-wrap items-center justify-center gap-3">
        <UrgencyBadge level={ticket.urgency_level} />
        <span className="rounded-badge bg-[#E6F4F0] px-3 py-1 text-xs font-bold uppercase tracking-wider text-[#0D7A5F]">
          ● {ticket.queue_status.replace('_', ' ')}
        </span>
      </div>
      {seeing ? (
        <p className="rounded-card bg-[#0D7A5F] px-4 py-3 text-sm font-bold text-white">
