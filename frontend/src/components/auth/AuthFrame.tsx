...
export function AuthFrame({ eyebrow, title, description, children }: { eyebrow: string; title: string; description: string; children: ReactNode }) {
  return (
    <main className="min-h-screen bg-[#f4f5ef] p-3 sm:p-5">
      <div className="mx-auto grid min-h-[calc(100vh-1.5rem)] max-w-[1440px] overflow-hidden rounded-[2rem] bg-white shadow-[0_30px_100px_rgba(7,61,51,.12)] lg:min-h-[calc(100vh-2.5rem)] lg:grid-cols-[1.08fr_.92fr]">
        ...
        <section className="flex min-h-full flex-col p-6 sm:p-10 lg:p-14 xl:p-20">
          ...
          <div className="my-auto py-10"><p className="sv-kicker">{eyebrow}</p><h1 className="mt-4 max-w-xl font-display text-4xl leading-tight text-[#10231e] sm:text-5xl">{title}</h1><p className="mt-4 max-w-lg leading-7 text-[#60706a]">{description}</p><div className="mt-9">{children}</div></div>
        </section>
      </div>
    </main>
  );
